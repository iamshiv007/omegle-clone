...
const TermsModal = ({ setIsTermsModal }) => {
    ...
    return (
        <div style={{ width: "100%", height: "100vh", background: "rgba(0, 0, 0, 0.2)", display: "flex", justifyContent: "center", position: 'fixed', top: "0", left: "0", overflow: "hidden" }}>
            <div className='termsBox' style={{ padding: "20px", marginTop: "10%", height: "fit-content", background: "white", borderRadius: "10px" }}>
                <div style={{ display: "flex", flexDirection: "row-reverse", position: "relative" }}>
                    <button style={{ fontSize: "20px", background: "white", border: "none", position: "absolute", top: "-20px", right:"-15px" }} onClick={() => setIsTermsModal(false)}><IoClose /></button>
                </div>
                <div>
                    <span>
                        <input type="checkbox" style={{ margin: "0px 10px 10px 0px", width: "18px", height: "18px" }} checked={check1} onChange={() => setCheck1(!check1)} />
                    </span>
